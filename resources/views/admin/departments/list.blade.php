        ...
        <div class="clearfix">
          <ul class="nav navbar-right panel_toolbox">
            <button class="btn btn-danger" onclick="window.location='{{url('/department/new')}}'">
              <img src="{{asset('/master/build/icons/plus.svg')}}" width="13px" class="mr-5 mt--2"><a class="text-white">Thêm mới</a>
            </button>
          </ul>
        </div>
      </div>
            ...
            <table class="table table-striped jambo_table bulk_action table-bordered">
              <thead>
                <tr class="headings">
                  ...
                  <th class="column-title" colspan="2"><span class="nobr">Thao tác</span></th>
                </tr>
              </thead>
              <tbody>
                ...
                <tr class="odd pointer">
                  ...
                  <td class="text-center width-3">
                    <form method="POST" action="/department/detail" name="viewDetail{{$item['id']}}">
                      ...
                      <a href="javascript:document.viewDetail{{$item['id']}}.submit();"><i class="fa fa-pencil"></i></a>
                      {{CsrfTokenUtil::csrfToken()}}
                    </form>
                  </td>
                  <td class="text-center width-3"><a class="remove_department cursor-pointer" data-id="{{$item['id']}}"><i class="fa fa-close"></i></a></td>
                </tr>
                @endforeach
                @if (count($result) === 0)
